...
def draw_cenefa(c, base_y, row, vigencia):
    top = base_y + HALF
...
def generar_pdf(filas, vigencia, output_path):
    cv = canvas.Canvas(output_path, pagesize=(PW, PH))
    total = len(filas)
    for i in range(0, total, 2):
        draw_cenefa(cv, HALF, filas[i], vigencia)
        if i + 1 < total:
            draw_cenefa(cv, 0, filas[i+1], vigencia)
        cv.showPage()
    cv.save()
